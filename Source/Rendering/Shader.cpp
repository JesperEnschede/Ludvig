...
namespace Ludvig
{
    namespace Rendering
    {
        ...
        GLuint Shader::compile(std::string _vertexCode, std::string _fragmentCode)
        {
        ...
        void Shader::set_mat4x4(const char *uniform, glm::mat4 matrix)
        {
            glUniformMatrix4fv(glGetUniformLocation(this->program, uniform),1,GL_FALSE, &matrix[0][0]);
        }

        void Shader::set_vec3(const char *uniform, glm::vec3 vector)
        {
            glUniform3f(glGetUniformLocation(this->program,uniform),vector.x,vector.y,vector.z);
        }

        void Shader::set_texture(const char *uniform, GLuint id)
        {
        ...
        void Shader::set_bool(const char *uniform, int boolean)
        {
